 ...
 * 
 * <p>
 * This version is modified by Roger Yi, remove the part can not compiled with
 * JavaMe SDK.
 * </p>
 * 
...
public final class StringUtils {
	...
	public static boolean isBlank(String str) {
		...
		for (int i = 0; i < strLen; i++)
			if (CharUtils.isWhitespace(str.charAt(i)) == false)
				return false;
		return true;
	}
	...
	public static String[] substringsBetween(String str, String open,
		...
		while (pos < strLen - closeLen) {
			...
			vector.addElement(str.substring(start, end));
			pos = end + closeLen;
		}
		if (vector.isEmpty())
			return null;
	 ...
	 *             {@link #substringBetween(String, String)}.
	 *             Method will be removed in Commons Lang 3.0.
	 */
	 ...
	 *            treated as empty token separators; if <code>false</code>,
	 *            adjacent
	 *            separators are treated as one separator.
	 * @return an array of parsed Strings, <code>null</code> if null String
	 *         input
			...
			if (end > -1) {
				if (end > beg) {
					...
					if (numberOfSubstrings == max) {
						...
						substrings.addElement(str.substring(beg));
					}
					else {
						// The following is OK, because String.substring( beg, end ) excludes
						// the character at the position 'end'.
						substrings.addElement(str.substring(beg, end));
	...
	private static String[] splitByCharacterType(String str, boolean camelCase) {
		...
		Vector vector = new Vector();
		int tokenStart = 0;
		int currentType = CharUtils.getType(c[tokenStart]);
		for (int pos = tokenStart + 1; pos < c.length; pos++) {
			int type = CharUtils.getType(c[pos]);
			if (type == currentType) {
				continue;
			}
			if (camelCase && type == CharUtils.LOWERCASE_LETTER &&
				currentType == CharUtils.UPPERCASE_LETTER) {
				int newTokenStart = pos - 1;
				if (newTokenStart != tokenStart) {
					vector.addElement(new String(c, tokenStart, newTokenStart -
						tokenStart));
					tokenStart = newTokenStart;
				}
			}
			else {
				vector.addElement(new String(c, tokenStart, pos - tokenStart));
				tokenStart = pos;
			}
			currentType = type;
		}
		vector.addElement(new String(c, tokenStart, c.length - tokenStart));
		String[] strArray = new String[vector.size()];
		vector.copyInto(strArray);
		return strArray;
	}
	...
	public static String join(Object[] array, String separator, int startIndex,
		...
		bufSize *= (array[startIndex] == null
			? 16 : array[startIndex].toString().length()) +
			separator.length();
	 ...
	 *            until there are no more possible replacements or timeToLive <
	 *            0
	 * @param timeToLive
	...
	public static String capitalize(String str) {
		...
		return new StringBuffer(strLen).append(
			CharUtils.toTitleCase(str.charAt(0))).append(str.substring(1)).toString();
	}
	...
	public static String uncapitalize(String str) {
		...
		if (str == null || (strLen = str.length()) == 0)
			return str;
		return new StringBuffer(strLen).append(
			CharUtils.toLowerCase(str.charAt(0))).append(str.substring(1)).toString();
	}
	...
	public static String swapCase(String str) {
		...
		for (int i = 0; i < strLen; i++) {
			...
			else if (CharUtils.isTitleCase(ch)) {
				ch = CharUtils.toLowerCase(ch);
			}
			else if (CharUtils.isLowerCase(ch)) {
				ch = CharUtils.toUpperCase(ch);
			}
			buffer.append(ch);
		}
	...
	public static boolean isAlpha(String str) {
		...
		int sz = str.length();
		for (int i = 0; i < sz; i++)
			if (CharUtils.isLetter(str.charAt(i)) == false)
				return false;
		return true;
	}
	...
	public static boolean isAlphanumeric(String str) {
		...
		int sz = str.length();
		for (int i = 0; i < sz; i++)
			if (CharUtils.isLetterOrDigit(str.charAt(i)) == false)
				return false;
		return true;
	}
	...
	public static boolean isNumeric(String str) {
		...
		int sz = str.length();
		for (int i = 0; i < sz; i++)
			if (CharUtils.isDigit(str.charAt(i)) == false)
				return false;
		return true;
	}
	...
	public static boolean isAllLowerCase(String str) {
		...
		int sz = str.length();
		for (int i = 0; i < sz; i++)
			if (CharUtils.isLowerCase(str.charAt(i)) == false)
				return false;
		return true;
	}
	...
	public static boolean isAllUpperCase(String str) {
		...
		int sz = str.length();
		for (int i = 0; i < sz; i++)
			if (CharUtils.isUpperCase(str.charAt(i)) == false)
				return false;
		return true;
	}
	 ...
	 *             This method is broken as the join doesn't know which char to
	 *             use.
	 *             Method will be removed in Commons Lang 3.0.
	 * 
	 */
